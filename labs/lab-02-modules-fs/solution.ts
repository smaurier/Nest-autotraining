...
async function findJsFiles(dirPath: string, basePath: string = dirPath): Promise<string[]> {
...
function getConfig(defaults: Record<string, string>): Record<string, string> {
  ...
  for (const [key, defaultValue] of Object.entries(defaults)) {
    config[key] = process.env[key] !== undefined ? process.env[key] as string : defaultValue;
  }
  return config;
}
