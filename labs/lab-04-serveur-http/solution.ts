...
import { createTestRunner, startServer, httpGet, httpPost, httpPut, httpDelete } from '../test-utils.ts';
...
async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (method === 'POST' && !id) {
    ...
    const newUser: User = { id: nextId++, name: body.name as string, email: body.email as string };
    users.push(newUser);
  ...
  if (method === 'PUT' && id) {
    ...
    users[userIndex] = { ...users[userIndex], ...(body as Partial<User>) };
