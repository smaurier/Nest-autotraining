...
import { createTestRunner, sleep } from '../test-utils.ts';
...
async function fetchAllParallel(tasks: (() => Promise<string>)[]): Promise<string[]> {
...
async function fetchFirstResolved(tasks: (() => Promise<string>)[]): Promise<string> {
...
async function allSettledManual(promises: Promise<unknown>[]): Promise<SettledResult[]> {
  return Promise.all(
    promises.map(promise =>
      Promise.resolve(promise)
        .then((value): SettledResult => ({ status: 'fulfilled', value }))
        .catch((reason): SettledResult => ({ status: 'rejected', reason }))
    )
  );
}
...
class TaskRunner extends EventEmitter {
  async run(taskName: string, taskFn: () => Promise<unknown>): Promise<unknown> {
  ...
  async runAll(tasks: Task[]): Promise<(unknown | null)[]> {
...
await test('fetchAllParallel propage les erreurs', async () => {
  ...
  try {
    ...
    caught = true;
    assertEqual((err as Error).message, 'task failed', 'L\'erreur doit etre propagee');
  }
...
await test('allSettledManual reproduit Promise.allSettled', async () => {
  ...
  assertEqual((results[0] as { status: 'fulfilled'; value: unknown }).value, 'success-1', 'Premier: valeur correcte');

  assertEqual(results[1].status, 'rejected', 'Deuxieme: rejected');
  assertEqual(((results[1] as { status: 'rejected'; reason: unknown }).reason as Error).message, 'fail-1', 'Deuxieme: raison correcte');

  assertEqual(results[2].status, 'fulfilled', 'Troisieme: fulfilled');
  assertEqual((results[2] as { status: 'fulfilled'; value: unknown }).value, 'success-2', 'Troisieme: valeur correcte');

  assertEqual(results[3].status, 'rejected', 'Quatrieme: rejected');
  assertEqual(((results[3] as { status: 'rejected'; reason: unknown }).reason as Error).message, 'fail-2', 'Quatrieme: raison correcte');
});
...
await test('TaskRunner emet start et complete', async () => {
  ...
  runner.on('start', (data: Record<string, unknown>) => events.push({ type: 'start', ...data }));
  runner.on('complete', (data: Record<string, unknown>) => events.push({ type: 'complete', ...data }));
...
await test('TaskRunner runAll execute les taches sequentiellement', async () => {
  ...
  runner.on('start', (data: Record<string, unknown>) => events.push({ type: 'start', ...data }));
  runner.on('progress', (data: Record<string, unknown>) => events.push({ type: 'progress', ...data }));
  runner.on('complete', (data: Record<string, unknown>) => events.push({ type: 'complete', ...data }));
  runner.on('error', (data: Record<string, unknown>) => events.push({ type: 'error', ...data }));
